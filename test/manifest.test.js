...
test(
  "buildManifest writes entries and can be read back",
  { concurrency: false },
  async () => {
    const originalCwd = process.cwd();
    const tmpDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "sandboxify-manifest-"),
    );
    ...
    try {
      writeFile(
        ...
        JSON.stringify(
          { name: "manifest-fixture-app", type: "module", private: true },
          null,
          2,
        ),
      );
      ...
      assert.ok(
        Array.isArray(manifest.entriesBySpecifier["fixture-lib"].exportNames),
      );
      assert.ok(
        manifest.entriesBySpecifier["fixture-lib"].exportNames.includes(
          "default",
        ),
      );
      assert.ok(
      ...
      assert.deepEqual(
        loaded.entriesBySpecifier["fixture-lib"].exportNames.sort(),
        ["default", "named"],
      );
    } finally {
