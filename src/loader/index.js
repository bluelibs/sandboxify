...
export function createSandboxHooks({
  ...
  return {
    resolve(specifier, context, nextResolve) {
      ...
      debugLog('resolve', { specifier, bucket, realUrl: resolved.url, id });

      return {
        shortCircuit: true,
        url: `sandboxify:${encodeURIComponent(bucket)}:${id}`,
      };
    },
    ...
    load(url, context, nextLoad) {
      ...
      const parsed = parseSandboxifyUrl(url);
      ...
      const manifestEntry = getManifestEntry(manifest, record.realUrl, record.specifier);
      const exportNames = normalizeExportNames(manifestEntry?.exportNames);
      const source = generateStubSource({
      ...
      debugLog('load', { bucket: record.bucket, specifier: record.specifier, realUrl: record.realUrl, exportNames });
...
function generateStubSource({ bucket, specifier, realUrl, exportNames }) {
  const namedExports = exportNames.filter((exportName) => exportName !== 'default' && isValidIdentifier(exportName));
  ...
  for (const exportName of namedExports) {
    lines.push(`export const ${exportName} = __sandboxifyModule[${JSON.stringify(exportName)}];`);
  }
...
function parseSandboxifyUrl(url) {
  const match = /^sandboxify:([^:]+):(.+)$/.exec(url);
  if (!match) {
    throw new Error(`Invalid sandboxify URL: ${url}`);
  }
