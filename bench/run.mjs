...
const output = {
  ...
  benchmarkConfig: {
    ipcBlobThresholdBytes:
      process.env.SANDBOXIFY_IPC_BLOB_THRESHOLD_BYTES ?? null,
  },
  runs,
};
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("exit", (code) => {
      if (code !== 0) {
        reject(
          new Error(
            `[bench] worker failed (${scenario}/${mode}): ${stderr || stdout}`,
          ),
        );
        return;
      }
      ...
      if (!line) {
        reject(
          new Error(
            `[bench] worker produced no JSON output (${scenario}/${mode})`,
          ),
        );
        return;
      }
      ...
      try {
        ...
        reject(
          new Error(
            `[bench] invalid worker JSON (${scenario}/${mode}): ${error.message}\n${line}`,
          ),
        );
      }
    });
